...
def test_valid_baked_good():
    ...
    assert baked_good.purchasing_cost == Decimal("10.00")
    assert baked_good.retail_price == Decimal("15.00")
...
def test_name_cannot_have_leading_or_trailing_whitespace():
    """
    Tests that a non-empty name with leading/trailing whitespace is rejected.
    """
    with pytest.raises(ValidationError):
        BakedGood(
            active=True,
...
def test_valid_baked_good_update():
    ...
    assert baked_good.purchasing_cost == Decimal("3.00")
    assert baked_good.retail_price == Decimal("7.00")
...
def test_baked_good_update_requires_all_fields():
    """
    Tests that BakedGoodUpdate requires every field.
    """
    with pytest.raises(ValidationError):
        BakedGoodUpdate(
            name="Chocolate Cake",
